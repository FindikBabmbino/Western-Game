...
class Coffin(Entity,Monster):
	...
	def attacking(self):
		...
		if distance<self.attack_radius and not self.attack:
			self.attack=True
			self.frame_index=0
	...
	def animate(self,dt):
   		...
   		if int(self.frame_index)==4 and self.attack:
...
class Cactus(Entity,Monster):
	...
	def animate(self,dt):
		...
		if int(self.frame_index)==6 and self.attack and not self.bullet_shot:
